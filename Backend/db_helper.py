    ...
    with get_db_cursor() as cursor:
        cursor.execute(
            '''SELECT month(expense_date) as expense_month, 
               ...
               sum(amount) as total_spent FROM expenses
...
def fetch_month_category_summary():
    logger.info(f"fetch_category_summary_by_months")
    with get_db_cursor() as cursor:
        cursor.execute(
            '''SELECT DATE_FORMAT(expense_date, '%Y-%m') AS month, 
               category, SUM(amount) AS total_spent FROM expenses
...
def fetch_top_expenses():
    logger.info(f"fetch_top_expenses_by_amount")
    with get_db_cursor() as cursor:
        cursor.execute(
            '''SELECT expense_date, category, amount, notes 
               FROM expenses ORDER BY amount DESC
               LIMIT 10;
            '''
        )
        data = cursor.fetchall()
